const loadCategaryData=async ()=>{

    const res=await fetch('https://openapi.programming-hero.com/api/peddy/categories');
  
    const data=await res.json(); 
  
    const activeBackground=false;
  ...
  const searchPetByCategoris= async (searchCategory)=>{ 
    ...
    const res=await fetch(`https://openapi.programming-hero.com/api/peddy/category/${searchCategory}`);
    ...
    const notAvailave=document.getElementById('not-availve');
  ...
  const showModal= async (data)=>{ 
    ...
    const res=await fetch(`https://openapi.programming-hero.com/api/peddy/pet/${data}`);
  
    const resData=await res.json();
  
    const details=document.getElementById('show_imformation');
